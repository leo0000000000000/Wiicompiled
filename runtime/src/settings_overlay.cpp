...
void DrawRebindPrompt() {
    ...
    if (!g_rebind.active) {
        ...
        g_rebindFromGuide = false; // Reset the context flag when modal closes
    }
    ImGui::EndPopup();
}
...
/**
 * @brief Handles text input resizing for expression configuration strings.
 */
...
/**
 * @brief Renders the Dolphin-style expression configuration settings.
 */
...
/**
 * @brief Renders the global controller rumble settings.
 */
...
/**
 * @brief Renders the main controller settings panel, handling assignments and mappings.
 */
...
/**
 * @brief Renders the global audio configuration menu.
 */
...
/**
 * @brief Renders the framerate and timing overlay.
 */
void DrawFpsOverlay() {
...
/**
 * @brief Renders the shader compilation status overlay.
 */
...
/**
 * @brief Renders the initial loading and startup screen.
 */
...
void DrawKeyboardVisualGuide() {
    ...
    if (ImGui::Begin("Keyboard Controls Setup", &g_showKeyboardGuide, flags)) {
        ...
        if (g_rebindFromGuide) {
            DrawRebindPrompt();
            if (!g_rebind.active) g_rebindFromGuide = false;
        }
    }
    ImGui::End();
}
...
void DrawTopBar() {
    ...
    if (ImGui::BeginMenu("Controller settings")) {
        ...
        // Preserves modal capture under the settings popup context
        if (!g_rebindFromGuide) {
            DrawRebindPrompt();
        }
        ImGui::EndMenu();
...
/**
 * @brief Checks if an SDL event is a toggle key press.
 * @param event The SDL event to evaluate.
 * @param code The target scancode to match against.
 * @return True if the event matches the specified toggle key.
 */
...
/**
 * @brief Determines if an SDL event represents mouse activity.
 * @param event The SDL event to evaluate.
 * @return True if the event is a mouse motion, button, or wheel action.
 */
...
/**
 * @brief Updates the visibility of the mouse cursor based on user activity timeout.
 */
...
/**
 * @brief Persists the display mode to the configuration if modified externally.
 */
