...
void DrawGraphicsSettings() {
    ...
    ImGui::Separator();
    /* Disabled, due to feedback loop bug and redundancy: UI scale slider is already present in Controls Guide.
    // UI scale setting in Graphics menu (clamped to persistent range)
    ImGui::SetNextItemWidth(200.0f);
    ImGui::SliderFloat("UI Scale", &g_userUiScale, 0.75f, 2.50f, "%.2fx");
    if (ImGui::IsItemDeactivatedAfterEdit()) {
...
/**
 * @brief Renders the confirmation modal when quitting the game, scaled dynamically.
 */
void DrawExitPrompt() {
    ...
    // Apply UI scale to popup text and controls
    ImGui::SetWindowFontScale(scale);

    ImGui::TextUnformatted("Quit the game?");
    ImGui::Spacing();

    const float btnWidth = 120.0f * scale;
    if (ImGui::Button("Exit", ImVec2(btnWidth, 0.0f))) ExitForAuroraWindowClose();
    ImGui::SameLine();
    if (ImGui::Button("Cancel", ImVec2(btnWidth, 0.0f))) g_exitPromptOpen = false;
...
void DrawTopBar() {
 ...
 const ImGuiStyle& style = ImGui::GetStyle();
    ...
    ImGui::SetCursorPosX(std::max(ImGui::GetCursorPosX(),
                                  ImGui::GetWindowWidth() - hideWidth - exitWidth - style.ItemSpacing.x - (8.0f * scale)));
